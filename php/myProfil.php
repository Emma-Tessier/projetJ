...
if (empty($cara_id)) {
    header('location: profil.php?id=' . $voir['id'] .'');
    // permet de ne plus accéder à cette page
    exit;
}
$req = $bdd->prepare("SELECT v.fname, v.age, v.ville, c.* FROM visiteurs v  LEFT JOIN caracteristiques c ON  v.caracteristiques_id = c.id where v.id = ?");
$req->execute(array($cara_id));
$voir = $req->fetch();
